...
IB_CPP_SOURCES := src/InteractiveBrokers/InteractiveBrokersGateway.cpp \
...
.PHONY: all build run clean check_ib_python check_ib_api check_protobuf ib_gateway ib_build ib_gateway_cpp ib_processor historical_store

all: build

build: $(RNG_TARGET) $(IB_PROCESSOR_TARGET)
...
clean:
	rm -f $(RNG_TARGET) $(IB_PROCESSOR_TARGET) $(IB_HISTORICAL_STORE_TARGET) $(IB_CPP_TARGET)